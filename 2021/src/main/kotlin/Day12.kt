...
private fun getPossibilities(choices: MutableList<String>, nextChoice: String, map: Map<String, MutableSet<String>>): MutableSet<MutableList<String>> {
    choices.add(nextChoice)
    if (nextChoice == end) return mutableSetOf(choices);

    val nextOptions = map.getOrDefault(nextChoice, emptyList())
...
private fun getPossibilitiesPt2(choices: MutableList<String>, nextChoice: String, map: Map<String, MutableSet<String>>): MutableSet<MutableList<String>> {
    choices.add(nextChoice)
    if (nextChoice == end) return mutableSetOf(choices);

    val nextOptions = map.getOrDefault(nextChoice, emptyList()).filterNot { it == start }
...
private fun partOne(pt: Int = 1) {
    val input = InputUtil.readFileAsStringList("day12/input.txt")
