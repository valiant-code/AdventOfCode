...
private fun partOne(pt: Int = 1) {
    val input = InputUtil.readFileAsStringList("day11/input.txt")
                ...
                .forEach {
                    if (it.flash(currentStep)) {
                        getAdjacentAndDiagonalPoints(it.coordinate).forEach { point ->
                            map.getOrDefault(point, null)?.incrementEnergy(currentStep)
                        }
                    }
                }
        }
        ...
        while (map.values.any { it.readyToFlash(step) }) {
            map.values.filter { it.readyToFlash(step) }
                .forEach {
                    if (it.flash(step)) {
                        getAdjacentAndDiagonalPoints(it.coordinate).forEach { point ->
                            map.getOrDefault(point, null)?.incrementEnergy(step)
                        }
                    }
                }
        }
