...
public class Day21 {
    ...
    public static void partOne() throws IOException {
        ...
        List<Pair<List<String>, List<String>>> ingredientsWithAllergens = input.stream().map(line -> {
...
//        }
        ...
        allAllergens.forEach(allergen -> {
            Set<List<String>> listsItAppearsIn = ingredientsWithAllergens.stream().filter(p -> p.getLeft().contains(allergen)).map(Pair::getRight).collect(Collectors.toSet());
